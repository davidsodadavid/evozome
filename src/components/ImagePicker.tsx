...
import { constraintsFor } from "@/lib/uploads";
...
export default function ImagePicker({
  ...
  const { allowed, maxBytes, maxMb } = constraintsFor("photo");
      ...
      {libraryOpen && (
        <div className="rounded-md border border-white/10 bg-white/5 p-3">
          {mediaLibrary.length === 0 ? (
            <p className="text-xs text-white/50">No photos uploaded yet — use &ldquo;Upload new&rdquo;.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {mediaLibrary.map((m) => (
                <button
